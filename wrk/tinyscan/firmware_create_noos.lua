...
function init(args)
	...
	local f = io.open("/root/wrk/tinyscan/noos.bin","rb")
	local len = f:seek("end")
	f:seek("set",0)
	local bin = f:read(len)
	
	file = [[--]]..boundary..cr..[[Content-Disposition: form-data; name="firmware"; filename="noos.bin"]]..cr..[[Content-Type: application/octet-stream]]..cr..cr..bin
